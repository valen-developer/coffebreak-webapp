...
export class CustomReuseStrategy implements RouteReuseStrategy {
  ...
  shouldAttach(route: ActivatedRouteSnapshot): boolean {
    ...
    const canAttach: boolean = !!route.routeConfig && !!this.storedRoutes[path];
    if (!canAttach) return false;

    const paramsMatch: boolean = this.compareObjects(
      route.params,
      this.storedRoutes[path].snapshot.params
    );
    const queryParamsMatch: boolean = this.compareObjects(
      route.queryParams,
      this.storedRoutes[path].snapshot.queryParams
    );
  ...
  retrieve(route: ActivatedRouteSnapshot): DetachedRouteHandle {
    const { routeConfig } = route;
    if (!routeConfig) return null as any;

    const { path } = routeConfig;
    if (!path) return null as any;

    // return null if the path does not have a routerConfig OR if there is no stored route for that routerConfig
    if (!this.storedRoutes[path]) return null as any;
